...
def addStudent(data):
    ...
    if "requestedPartners" in data:
        ...
        for name in work_with:
            ...
            student_to_add.update(add_to_set__temp_work_with=ident)
#            student_to_add.update(add_to_set__temp_work_with=name)
    if "bannedPartners" in data:
        ...
        for name in dont_work_with:
            ident = convertNameToId(name)
            student_to_add.update(add_to_set__temp_dont_work_with=ident)
...
def removeGroup(group, students, groups):
...
def unpopular(students, groups):
    for group in groups:
        if len(group.preferences) < MIN_SIZE:
            ...
            students, groups = removeGroup(group, students, groups)
    return students, groups
...
#                           reverse=True)
    ...
    matched, groups = swapController(matched, groups)
    warnLeaders(matched)
    for group in groups:
...
def swapController(matched, groups):
    for group in matched:
        if len(group.members) < MIN_SIZE:
            groups = swapThemBitches(group, True, groups)
            if len(group.members) < MIN_SIZE:
                groups = swapThemBitches(group, False, groups)
...
def removeExtraGroups():
    opt_groups = Groups.objects(option=True)
    opt_dict = {}
        ...
        for group in del_list:
            ...
            for student in Students.objects:
                if group in student.preferences:
                    ...
                    student.preferences.update(pull__x=group)
                    student.save()
...
#        group.modify(members=[], preferences={})
#        l_groups.append(group)
    l_students, l_groups = unpopular(l_students, l_groups)
